...
/**
 * EVALUAR SOLICITUD (Aprobar o Rechazar)
...
export const evaluarSolicitud = async (req, res) => {
  try {
    ...
    const practica = await practicaRepo.findOne({
      where: { id },
      relations: ["formularioRespuestas", "student", "empresaToken", "empresa"],
    });
    ...
    if (decision === "aprobar") {
      ...
      if (respuestaForm) {
        ...
        respuestaForm.comentario_coordinador =
          observaciones || "Solicitud aprobada exitosamente.";
    ...
    /**
     * MENSAJERÍA INTERNA AUTOMÁTICA
     * Si es un rechazo hacia la empresa, creamos un hilo en el chat automáticamente
     */
    try {
      if (
        decision === "rechazar" &&
        (destinatario === "empresa" || destinatario === "ambos")
      ) {
        const asunto = "Observación de práctica – correcciones requeridas";
        const contenido = `Se ha observado la documentación de la práctica del alumno ${
        ...
        const destinatarioEmail =
          practica.empresa?.email || practica.empresaToken?.empresaCorreo;
      ...
      // si falla el mensaje de chat, no queremos que falle toda la aprobación
