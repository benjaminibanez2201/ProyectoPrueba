...
const ResponderBitacora = () => {
    ...
    const cargarDatos = async () => {
      try {
        ...
        if (practicaData.estado === "cerrada") {
          showErrorAlert(
            "Práctica Cerrada",
            "No puedes crear Bitácoras porque tu práctica ya está cerrada."
          );
          navigate("/panel");
          return;
        }
        ...
        if (practicaData.estado !== "en_curso") {
          showErrorAlert(
            "Acceso denegado",
            'Solo puedes crear Bitácoras si la práctica está "En Curso".'
          );
          navigate("/panel");
          return;
        }
        ...
        showErrorAlert(
          "Error",
          "No se encontró una práctica activa para el usuario o fallo la conexión."
        );
        navigate("/panel");
      } finally {
        setLoading(false);
  ...
  const handleFormSubmit = async (respuestas) => {
    ...
    if (!practicaId) {
      showErrorAlert(
        "Error",
        "No se puede enviar, ID de práctica no encontrado."
      );
      return;
    }
    try {
      ...
      await showSuccessAlert(
        "Bitácora Enviada",
        "Los datos han sido guardados y registrados."
      );

      // Navegamos *después* del éxito para que el usuario pueda ver la alerta.
      navigate("/panel");
    } catch (err) {
      // En caso de error de red o backend (ej. 500, 401, 400)
      showErrorAlert(
        "Error al guardar",
        err.message || "Error desconocido al enviar la Bitácora."
      );
    } finally {
      setProcesando(false);
    }
  ...
  if (!plantilla)
    return (
      <div className="text-center p-8">
        No se encontró la plantilla de Bitácora. Contacte al coordinador.
      </div>
    );
