...
export async function getConversacionPracticaConToken(practicaId, token) {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error al cargar la conversación"
    );
  }
}
...
export async function getBandejaEntrada() {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error al cargar la bandeja"
    );
  }
}
...
export async function getMensajesEnviados() {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error al cargar mensajes enviados"
    );
  }
}
...
export async function marcarComoLeido(mensajeId, token = null) {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error al marcar como leído"
    );
  }
}
...
export async function getNoLeidosEmpresa(practicaId, token) {
  try {
    const response = await axios.get(
      `/comunicacion/no-leidos-empresa/${practicaId}`,
      {
        params: { token },
      }
    );
