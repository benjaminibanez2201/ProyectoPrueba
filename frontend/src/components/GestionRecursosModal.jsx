...
import { Upload, Trash2, FileText, X, Layout, FolderOpen, AlertCircle } from 'lucide-react';
// Asegúrate de importar tu instancia de axios configurada
// Si tu archivo se llama diferente (ej: axios.js o user.service.js), ajusta esta línea
...
const GestionRecursosModal = ({ isOpen, onClose }) => {
    ...
    const fetchPlantillas = async () => {
        try {
            // AJUSTA ESTA RUTA si tu endpoint de plantillas es diferente
            const res = await instance.get('/forms/plantillas'); 
            setPlantillas(res.data.data || res.data);
        } catch (err) {
            console.error("Error al cargar plantillas. Tal vez la ruta no existe aún.", err);
        }
    };
    ...
    const handleDeleteRecurso = async (id) => {
        if (!confirm("¿Estás seguro de borrar este archivo? Los alumnos ya no podrán descargarlo.")) return;
                ...
                <div className="bg-gradient-to-r from-purple-700 to-purple-900 p-5 flex justify-between items-center text-white shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-purple-200 text-xs">Gestiona lo que ven los alumnos</p>
                        </div>
                    </div>
                ...
                <div className="flex border-b border-gray-200 bg-gray-50 shrink-0">
                    <button 
                        ...
                        className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-all relative
                            ${activeTab === 'archivos' ? 'text-purple-700 bg-white' : 'text-gray-500 hover:text-purple-600 hover:bg-gray-100'}`}
                    >
                        <FileText size={18} /> 
                        Archivos y Pautas
                        {activeTab === 'archivos' && <div className="absolute bottom-0 left-0 w-full h-1 bg-purple-700 rounded-t-full"></div>}
                    </button>
                    <button 
                        onClick={() => setActiveTab('plantillas')}
                        className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-all relative
                            ${activeTab === 'plantillas' ? 'text-orange-600 bg-white' : 'text-gray-500 hover:text-orange-600 hover:bg-gray-100'}`}
                    >
                        <Layout size={18} /> 
                        Formularios Digitales
                        {activeTab === 'plantillas' && <div className="absolute bottom-0 left-0 w-full h-1 bg-orange-500 rounded-t-full"></div>}
                    </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto grow bg-gray-50/50">
                    ...
                    {activeTab === 'archivos' && (
                        <div className="space-y-6">
                            ...
                            {/* Lista */}
                            <div>
                                <h4 className="font-bold text-gray-700 mb-3 ml-1 text-sm uppercase tracking-wide">Documentos Disponibles</h4>
                                <div className="space-y-2">
                                    {recursos.map(rec => (
                                        <div key={rec.id} className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 hover:border-purple-300 hover:shadow-md transition group">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-gray-800">{rec.nombre}</p>
                                                    <p className="text-xs text-gray-500">Subido el {new Date(rec.fecha_subida).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                            <button 
                        ...
                        <div className="space-y-4">
                            <div className="bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg">
                                <p className="text-sm text-orange-800">
                                    Estos son los formularios activos. Para editarlos, ve a la sección <strong>"Formularios"</strong> del dashboard.
                                </p>
                            </div>

                            <div className="grid gap-3">
                                {plantillas.map(plantilla => (
                                    <div key={plantilla.id} className="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition">
                                        <div className="bg-orange-100 p-3 rounded-xl shrink-0 text-orange-600">
                                            <Layout size={24} />
                                        </div>
                                        <div className="grow">
                                            <h5 className="font-bold text-gray-800">{plantilla.titulo}</h5>
                                            <p className="text-xs text-gray-500 mt-1">{plantilla.descripcion || "Sin descripción"}</p>
                                        </div>
                                        <div className="text-xs font-semibold px-3 py-1 bg-gray-100 rounded-full text-gray-600 border border-gray-200">
