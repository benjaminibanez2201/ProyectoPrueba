/**
 * COMPONENTE: CHAT DE MENSAJERÍA
 * Interfaz de conversación tipo "burbujas" que permite el intercambio de mensajes vinculados a una práctica específica
 */
...
import {
  getConversacionPractica,
  getConversacionPracticaConToken,
  enviarMensaje,
  marcarComoLeido,
} from "../services/mensaje.service.js";
import { showErrorAlert, showSuccessAlert } from "../helpers/sweetAlert.js";

const ChatMensajeria = ({
  practicaId,
  token,
  destinatarioId,
  usuarioActual,
  ...
  // Efecto: Carga la conversación inicial y marca como leídos los mensajes al abrir
  useEffect(() => {
    cargarConversacion();
  }, [practicaId]);

  // Efecto: Mantiene el scroll al final cada vez que llega un mensaje nuevo
  useEffect(() => {
    scrollToBottom();
  }, [mensajes]);
  ...
  /**
   * CARGAR CONVERSACIÓN
   * Determina si usar la API protegida por JWT o la API pública por token (empresa)
   */
  ...
  /**
   * ENVIAR MENSAJE
   * Prepara el objeto de datos incluyendo el destinatario (email) y el token si aplica
   */
  const handleEnviar = async (e) => {
    ...
    try {
      ...
      // Si no tenemos destinatarioId explícito, lo buscamos en el historial (el último que me escribió)
      const ultimoMensajeRecibido = [...mensajes]
        .reverse()
        .find((m) => m.remitente_email !== usuarioActual.email);
      ...
      const data = {
        ...
        destinatarioId:
          destinatarioId || ultimoMensajeRecibido?.remitente_email,
      };
      ...
      if (!data.destinatarioId && !token) {
        showErrorAlert(
          "Error",
          "No se pudo identificar el destinatario del mensaje"
        );
        setEnviando(false);
        return;
      }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-100">
                Comunicación sobre la práctica
              </p>
            </div>
          </div>
          <button
          ...
          {loading ? (
            ...
            mensajes.map((msg) => {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[70%] ${esMio ? "order-2" : "order-1"}`}
                  >
                    <p
                      className={`text-xs font-semibold mb-1 ${
                        esMio ? "text-right text-blue-600" : "text-gray-600"
                      }`}
                    >
        ...
        >
          <div className="mb-3">
            <input
              ...
              onChange={(e) =>
                setNuevoMensaje((prev) => ({ ...prev, asunto: e.target.value }))
              }
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={
                enviando ||
                !nuevoMensaje.asunto.trim() ||
                !nuevoMensaje.contenido.trim()
              }
